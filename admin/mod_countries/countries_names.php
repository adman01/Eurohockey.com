...
echo $head->setTitle(langGlogalTitle."Add alternative name to country");
...
<div id="main">
  ...
  <!--  /tray -->
	...
	<h1>Add alternative name to country</h1>
    <p class="box">
...
<?php
  ...
  <h2>Default name of country: <b><?php echo $write['name'];?></b></h2>
	<p class="msg info">If there is no Alternative name, the default country name will be used. Insert Alternative name for the season when it was changed only! It will be used for all future seasons, unless there is another one specified.</p>
<form action="countries_action.php" method="post" id="form-01">
<fieldset>
	<legend>Add alternative name to country</legend>
...
<?
    $query="SELECT * FROM countries_names WHERE id_country=".$_GET['id']." ORDER BY int_year DESC";
    //echo $query; 
    ...
    while($write = $result->fetch_array())
	  ...
	  if ($a%2) $style=""; else $style="";
  		 echo '<tr class="'.$style.'">
        <td class="high-bg">';
         if ($users->checkUserRight(3)) echo'<a href="countries_names_update.php'.Odkaz.'&amp;id='.$write["id"].'&amp;id_country='.$id.'&amp;filter='.$_GET['filter'].'&amp;filter2='.$_GET['filter2'].'&amp;list_number='.$_GET['list_number'].'" title="Edit item">';
         echo '<b>'.$write['name'].'</b>';
        ...
        echo'</td>';
        echo'<td>';
          if ($users->checkUserRight(3)) echo'<a href="countries_names_update.php'.Odkaz.'&amp;id='.$write["id"].'&amp;id_countries='.$id.'&amp;filter='.$_GET['filter'].'&amp;filter2='.$_GET['filter2'].'&amp;list_number='.$_GET['list_number'].'" title="Edit item"><img src="../inc/design/ico-edit.gif" class="ico" alt="Edit"></a>';
          echo '&nbsp;&nbsp;';
