    ...
    for ($i=0; $i<=($number); $i++){
      ...
      if (!empty($date) and !empty($id_stage) and !empty($id_home_team) and !empty($id_visiting_team) and !empty($games_status)){
        ...
        if (!$con->RunQuery($query)){
              ...
              $id_arena=$con->GetSQLSingleResult("SELECT id_arena as item FROM clubs_arenas_items WHERE id_club=".$id_home_team." ORDER BY bool_main_arena DESC, id_arena ASC");
              if (!empty($id_arena)) {
      ...
      $query="SELECT DISTINCT id,id_stage FROM standings WHERE id_type=1 AND id_season=".$id_season." AND id_league=".$id_league." ";
      $result = $con->SelectQuery($query);
      while($write = $result->fetch_array()){
           generate_auto_standings($write['id']);
           $xml->generate_xml_standings($write['id']);
      }

      $query="SELECT DISTINCT id_stage FROM games WHERE id_season=".$id_season." AND id_league=".$id_league." ";
      //echo $query; 
      $result = $con->SelectQuery($query);
      while($write = $result->fetch_array()){
          $xml->generate_xml_games($id_season,$id_league,$write['id_stage']);
      }
...
//pridat novy zaznam PERIODS
    if ($_POST['action']=="add_periods" and $users->checkUserRight(2)){
    ...
    for ($i=1; $i<=(5); $i++){
      ...
      if (!empty($order) and !empty($id_type) and !empty($id_game) and ($score_home!="NULL" OR $score_visiting!="NULL" OR $shoots_home!="NULL" OR $shoots_visiting!="NULL"  )){
        ...
        $xml->generate_xml_game_detail($id_game);
      ...
      header("Location: games_info.php".Odkaz."&message=1".$strReturnData."&intCorrect=".$intCorrect."&intError=".$intError);
}
...
//smazat zaznam / PERIODS
    ...
    $strReturnData='&id='.$_GET['id_item'].'&filter='.$_GET['filter'].'&filter2='.$_GET['filter2'].'&list_number='.$_GET['list_number'];
      ...
      if (isset($_GET['id'])){
        ...
        if ($con->RunQuery($query)==true){
          $xml->generate_xml_game_detail($intGameID);
    ...
    $strReturnData='&id='.$_GET['id_item'].'&filter='.$_GET['filter'].'&filter2='.$_GET['filter2'].'&list_number='.$_GET['list_number'].'&tab=2';
      ...
      if (isset($_GET['id'])){
        ...
        //$intIDstageDefault=$con->GetSQLSingleResult("SELECT id_detault as item FROM games_stages WHERE id=".$id_stage);
         ...
         $games_shotouts=$write_sub['games_shotouts'];
         ...
         if (!$boolStatsLock){
          ...
          if ($games_shotouts==1){$games->WriteStats($id_stats,'shotouts','-',1);}
          $games->WriteStats($id_stats,'games_dressed','-',1);
          if ($games_dressed==0){  $games->WriteStats($id_stats,'games','-',1);        }
...
//upravit zaznam  STATS
    ...
    for ($i=0; $i<$number; $i++){
      ...
      $games_shotouts=$_POST['games_shotouts'][$i];
      $games_shotouts=$input->check_number($games_shotouts);
      ...
      if (!empty($id) and !empty($id_club) and !empty($id_season) and !empty($id_league) and !empty($id_stage)){
        ...
        if (!$boolStatsLock){
          ...
          if ($con->GetQueryNum($query_sub)>0){
            ...
            $games_shotouts_old=$write_sub['games_shotouts'];
          ...
          if ($games_shotouts_old==1){$games->WriteStats($id_stats,'shotouts','-',1);}
        ...
        $query="UPDATE games_stats SET goals=".$goals.",assist=".$assist.",plusminus=".$plusminus.",games_dressed=".$games_dressed.",games_shotouts=".$games_shotouts.",shoots_against=".$shoots_against.",goals_against=".$goals_against.",penalty=".$penalty.",minutes=".$minutes." WHERE id=".$id;
