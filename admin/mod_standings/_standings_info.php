...
<div class="tabs box" id="tabs">
			<ul>
			    <? if ($id_type==1) {
            echo '<li><a href="#tab01"><span>Automatic - no groups</span></a></li>';
          }else{
...
function get_table_clubs ($counter,$id_group,$id_standings,$id_type){
  ...
  if ($intPocet>0){
    $strOrder=" points DESC,wins DESC,games ASC, (score1-score2) DESC, id DESC";
    ...
    while($write = $result->fetch_array())
	  {
       ...
       echo '</td>';
        ...
        if ($con->GetQueryNum($query_sub)>0){
          ...
          echo '<tr class="bg"><td colspan="10" class="low smaller t-center">'.$write_sub['name'].'</td>';
          echo '<td>';
      ...
      ';
         ...
         echo '<tr><td colspan="11"><p class="msg warning">no team found</p></td>';
         echo '</table>';
    }
  echo'
...
function generate_auto_standings($id_standings){
  ...
  //delete automatic table
  $query="DELETE FROM standings_teams WHERE id_table_type=1 AND id_standings=".$id_standings."";
  $con->RunQuery($query);
  ...
  $result = $con->SelectQuery($query);
    while($write = $result->fetch_array())
     ...
     $intWins1=$con->GetSQLSingleResult("SELECT count(*) as item FROM games WHERE id_club_home=".$write['id_club']." AND home_score>visiting_score AND games_status=1 AND ".$sqlWhere);
     $intWins2=$con->GetSQLSingleResult("SELECT count(*) as item FROM games WHERE id_club_visiting=".$write['id_club']." AND home_score<visiting_score AND games_status=1 AND ".$sqlWhere);
     ...
     $intLosts1=$con->GetSQLSingleResult("SELECT count(*) as item FROM games WHERE id_club_home=".$write['id_club']." AND home_score<visiting_score AND games_status=1 AND ".$sqlWhere);
     $intLosts2=$con->GetSQLSingleResult("SELECT count(*) as item FROM games WHERE id_club_visiting=".$write['id_club']." AND home_score>visiting_score AND games_status=1 AND ".$sqlWhere);
     ...
     $intDraws1=$con->GetSQLSingleResult("SELECT count(*) as item FROM games WHERE id_club_home=".$write['id_club']." AND home_score=visiting_score AND games_status=1 AND ".$sqlWhere);
     $intDraws2=$con->GetSQLSingleResult("SELECT count(*) as item FROM games WHERE id_club_visiting=".$write['id_club']." AND home_score=visiting_score AND games_status=1 AND ".$sqlWhere);
            ...
            if ($con->GetQueryNum($query)>0){
              while($write = $result->fetch_array())
                ...
                echo '<div id="tab0'.$counter.'">';
                         get_table_form($counter,$write['id'],$_GET['id']);
