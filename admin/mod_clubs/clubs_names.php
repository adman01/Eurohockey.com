...
echo $head->setTitle(langGlogalTitle."Add alternative name to club");
...
<?php
  ...
  ?>
	...
	<h1>Add alternative name to clubs</h1>
    <p class="box">
  ...
  <h2>Default name of clubs: <b><?php echo $write['name'];?></b></h2>
	<p class="msg info">If there is no Alternative name, the default club name will be used. Insert Alternative name for the season when it was changed only! It will be used for all future seasons, unless there is another one specified.</p>
<form action="clubs_action.php" method="post" id="form-01">
<fieldset>
	<legend>Add alternative name to club</legend>
...
<?
    $query="SELECT * FROM clubs_names WHERE id_club=".$_GET['id']." ORDER BY int_year DESC";
    //echo $query; 
    ...
    while($write = $result->fetch_array())
	  ...
	  if ($a%2) $style=""; else $style="";
  		 echo '<tr class="'.$style.'">
        <td class="high-bg">';
         if ($users->checkUserRight(3)) echo'<a href="clubs_names_update.php'.Odkaz.'&amp;id='.$write["id"].'&amp;id_club='.$id.'&amp;filter='.$_GET['filter'].'&amp;filter2='.$_GET['filter2'].'&amp;list_number='.$_GET['list_number'].'" title="Edit item">';
         echo '<b>'.$write['name'].'</b>';
        ...
        echo'</td>';
        echo'<td>';
          if ($users->checkUserRight(3)) echo'<a href="clubs_names_update.php'.Odkaz.'&amp;id='.$write["id"].'&amp;id_club='.$id.'&amp;filter='.$_GET['filter'].'&amp;filter2='.$_GET['filter2'].'&amp;list_number='.$_GET['list_number'].'" title="Edit item"><img src="../inc/design/ico-edit.gif" class="ico" alt="Edit"></a>';
          echo '&nbsp;&nbsp;';
