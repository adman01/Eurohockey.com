...
<?php
  if (!empty($_GET['id'])) {
  $query="SELECT id_club,(SELECT name FROM clubs WHERE clubs.id=clubs_names.id_club LIMIT 1) as name FROM clubs_names WHERE id=".$_GET['id'];
  ...
  ?>
    ...
    <p class="box">
    	 <a href="clubs_names.php<?echo Odkaz;?>&amp;filter=<?echo $_GET['filter'];?>&amp;filter2=<?echo $_GET['filter2'];?>&amp;list_number=<?echo $_GET['list_number'];?>&amp;id=<?echo $_GET['id_club'];?>"  class="btn-list"><span>back to list of alternative names</span></a>
...
<?php
  if (!empty($_GET['id'])) {
  $query="SELECT * FROM clubs_names WHERE id=".$_GET['id'];
  $result = $con->SelectQuery($query);
  $write = $result->fetch_array();
