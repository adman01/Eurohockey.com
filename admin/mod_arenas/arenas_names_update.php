...
<?php
  if (!empty($_GET['id'])) {
  $query="SELECT id_arena,(SELECT name FROM arenas WHERE arenas.id=arenas_names.id_arena LIMIT 1) as name FROM arenas_names WHERE id=".$_GET['id'];
  $result = $con->SelectQuery($query);
  $write = $result->fetch_array();
  $defaultName=$write['name'];
  $defaultID=$write['id_arena'];
  }
  ?>

	<h1>Edit alternative name</h1>
    <p class="box">
    	 <a href="arenas_names.php<?echo Odkaz;?>&amp;filter=<?echo $_GET['filter'];?>&amp;filter2=<?echo $_GET['filter2'];?>&amp;list_number=<?echo $_GET['list_number'];?>&amp;id=<?echo $_GET['id_arena'];?>"  class="btn-list"><span>back to list of alternative names</span></a>
    	 <a href="arenas_update.php<?echo Odkaz;?>&amp;filter=<?echo $_GET['filter'];?>&amp;filter2=<?echo $_GET['filter2'];?>&amp;list_number=<?echo $_GET['list_number'];?>&amp;id=<?echo $defaultID;?>"  class="btn-list"><span>back to editing default informations about "<? echo $defaultName;?>"</span></a>
	  </p>
...
<?php
  if (!empty($_GET['id'])) {
  $query="SELECT * FROM arenas_names WHERE id=".$_GET['id'];
  $result = $con->SelectQuery($query);
  $write = $result->fetch_array();
...
<fieldset>
	...
	<input type="hidden" name="id_arena" value="<?php echo $_GET['id_arena']?>" />
