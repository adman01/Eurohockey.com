...
if (!empty($id)){
  ...
  if (!empty($StrClubName)){
    $boolClub=true;
    $strHeader="Historical appearances on World championships and Olympic tournaments";
...
<div id="layout" class="longer">
  ...
  <div id="main">
      ...
      <div id="text">
         ...
         <?php
         echo '<div class="box normal bold" style="padding:10px">National team: <a href="/club/'.get_url_text($StrClubName,$id).'?season='.$intSeason.'&amp;league='.$intLeague.'" title="'.$StrClubName.'">'.$StrClubName.' '.$strSeason.'</a></div>';
         ?>
                  ...
                  while($write_player = $result_player->fetch_array()){
                    ...
                    echo '<tr class="dark">';
                      ...
                      $player_name=$write_player['name'].' '.$write_player['surname'];
  		                echo '<td colspan="'.(9+$intColspan).'" class="player" valign="top" style="font-size:1.1em"><a href="/player/'.get_url_text($player_name,$write_player['id_player']).'" title="Show player profile: '.$player_name.' '.$player_surname.'">'.$write_player['name'].' <b>'.$write_player['surname'].'</b></a>';
                      echo '</td>';
                    echo '</tr>';
                    
                    $query_stats="SELECT id_season,games,goals,assist,penalty,plusminus,id_club,id_league,id_season_type,minutes,AVG,PCE,shotouts,(goals+assist) as points from stats INNER JOIN clubs ON stats.id_club=clubs.id WHERE id_player=".$write_player['id_player']." AND is_national_team=2 AND (id_league=756 OR id_league=758) ORDER BY id_season DESC, id_season_type DESC, stats.id DESC";
                    if ($con->GetQueryNum($query_stats)>0){
                      ...
                      while($write = $result->fetch_array()){
                       ...
                       echo '<td class="club" valign="top">'.($write['id_season']-1).'-'.substr($write['id_season'],2,2).'</td>';
                       ...
                       echo '<tr><td>&nbsp;</td><td class="" colspan="9">No national team stats found for season '.$strSeason.'</td></tr>';
                    }
          ...
          //echo $query; 
              if ($con->GetQueryNum($query)>0){
                  ...
                  while($write_player = $result_player->fetch_array()){
                    ...
                    $query_stats="SELECT id_season,games,goals,assist,penalty,plusminus,id_club,id_league,id_season_type,minutes,AVG,PCE,shotouts from stats INNER JOIN clubs ON stats.id_club=clubs.id WHERE id_player=".$write_player['id_player']." AND is_national_team=2  AND id_stats_type=2 AND (id_league=756 OR id_league=758) ORDER BY id_season DESC, id_season_type DESC, stats.id DESC";
                    //echo $query_stats; 
                    ...
                    if ($con->GetQueryNum($query_stats)>0){
                      ...
                      while($write = $result->fetch_array()){
                       ...
                       echo '<td class="position" valign="top" style="width:25px">&nbsp;</td>';
                       echo '<td class="club" valign="top">'.($write['id_season']-1).'-'.substr($write['id_season'],2,2).'</td>'; 
                       echo '<td class="club" valign="top">';
                       echo get_flag(strtolower($StrClubCountry),15,11);
                       echo '<a href="/stats/club/'.$write['id_season'].'/'.get_url_text($StrClubName,$write['id_club']).$strUrlSeasonType.$strUrlGoalies.'" title="Show stats in selected season: '.$StrClubName.'">'.$StrClubName.'</a>';
